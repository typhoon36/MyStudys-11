...
public class GameMgr : MonoBehaviour
{
    ...
    [Header("--- Key_Txt ---")]
    public Text KeyMsg_Txt = null;

    [Header("Quest")]
    public Text Quest_Txt = null;
    ...
    void Start()
    {
        ...
        //## 문 
        m_FL_Tm_Text.text = "Mon("+ GameMgr.m_KillCount+ "/ 20 )" +GlobalValue.g_CurFloorNum + "층(도달:"
        ...
        if (GlobalValue.g_CurFloorNum < GlobalValue.g_BestFloorNum)
        {
            ShowDoor();
        }
    ...
    void Update()
    {
        ...
        // Escape 키를 눌러서 커서의 가시성을 토글
        if (Input.GetKeyDown(KeyCode.Escape))
        {
            Cursor.visible = !Cursor.visible;
            KeyMsg_Txt.text = "<ESC> : Lock_Mode";
        }
    ...
    public void MonsterDied()
    {
        if (m_KillCount == 20)
        {
            ...
            monsterPool.Clear();
            SpawnDiamond(new Vector3(0, 0, 0)); // 다이아몬드 스폰 위치 설정 필요
            m_LastFloor_Text.text = "Door Open";
            m_LastFloor_Text.gameObject.SetActive(false);

            Quest_Txt.gameObject.SetActive(true);
            Quest_Txt.text = "<color=#39A2AE>어딘가에 다이아몬드가 생성되었습니다!</color>";
            StartCoroutine(DiamondSpawnTimer()); // 다이아몬드 스폰 타이머 시작
        }
    }

    //## 다이아몬드 생성
    void SpawnDiamond(Vector3 a_Pos)
    {
        GameObject a_Dia = Instantiate(m_DiaItem, a_Pos, Quaternion.identity);
        a_Dia.transform.position = a_Pos;
        isGameOver = true;
        ShowDoor();
    }

    IEnumerator DiamondSpawnTimer()
    {
        float timer = 60.0f; // 예시 타이머 시간
        while (timer > 0)
        {
            yield return new WaitForSeconds(1.0f);
            timer--;
            m_FL_Tm_Text.text = "Mon("+ GameMgr.m_KillCount+ "/ 20 )" +GlobalValue.g_CurFloorNum + "층(도달:"
          + GlobalValue.g_BestFloorNum + "F)" + timer.ToString();
    ...
    {
        //게임 종료 시까지 무한 루프
        while (!isGameOver && m_KillCount < 20)
        {
