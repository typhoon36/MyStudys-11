...
public class GameMgr : MonoBehaviour
{
    ...
    [Header("WDmgText")]
    public Transform m_WDCanvas = null;
    public GameObject m_WDmgTxtPrefab = null;
    ...
    void Start()
    {
        ...
        maxMonster = a_CacMaxMon;

        m_MonLimit = 10 + a_CacMaxMon;
    ...
    void MissionUpdate()
    {
        ...
        if (0 < m_Floor_TimeOut)
        {
            m_LastFloorText.text  = "<color=#00ffff>다이아몬드가 맵 어딘가에 나타났습니다.</color>";
        }

        else
        {
            m_LastFloorText.text = "<color=#ff0000>("+m_KillCnt+ " / "+m_MonLimit+" Mon )"+
                "최종 100층</color>";
        }
    ...
    //## 몬스터 카운트 & 몬스터 상태 체크
    //bool CheckMonCnt(int idx)
    //{
    //    m_MonCurNum++;

    //    //## 마지막 스폰될 몬스터일시 다이아몬드 스폰
    ...
    //            {
    //                GameObject a_DiaObj = Instantiate(m_DiamondItem);

    //                a_DiaObj.transform.position = points[idx].position;
    //            }
    //        }

    //        m_Floor_TimeOut = 60.0f;
    ...
    public void W_SpawnDmgTxt(int cont, Vector3 a_WSPos, Color a_Color)
    {

        if (m_WDCanvas == null && m_WDmgTxtPrefab == null)
            return;

        GameObject a_DmgObj = Instantiate(m_WDmgTxtPrefab) as GameObject;
        a_DmgObj.transform.SetParent(m_WDCanvas, false);

        W_DmgTxt a_WDmgTxt = a_DmgObj.GetComponent<W_DmgTxt>();

        if (a_WDmgTxt != null)
            a_WDmgTxt.InitState(cont, a_WSPos, a_Color);
